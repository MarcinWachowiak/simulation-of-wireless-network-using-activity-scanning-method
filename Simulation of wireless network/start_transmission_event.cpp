...
StartTransmissionEvent::StartTransmissionEvent(int time,
...
StartTransmissionEvent::~StartTransmissionEvent()
{
}

void StartTransmissionEvent::execute()
{
	try
	{
		...
		// save the timestamp of first transmit attempt
		transmitter_->set_transmit_timestamp(simulation_->get_sim_clock());
		//deactivate probing
		...
		simulation_->get_logger()->debug("[{0}]\tStarting PACKET transmission TX id:{1}, time length:{2}", simulation_->get_sim_clock(), transmitter_->get_k_tx_id(),transmitter_->get_transmission_time());
	} catch(...)
	{
		simulation_->get_logger()->error("[{0}]\tStarting transmission failed TX id:{1}", simulation_->get_sim_clock(), transmitter_->get_k_tx_id());
	}
}

bool StartTransmissionEvent::isTriggered()
{
